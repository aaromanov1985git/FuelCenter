...
"""
from sqlalchemy import Column, Integer, String, Numeric, DateTime, Date, Index, ForeignKey, Text, Boolean
...
class Transaction(Base):
    ...
    organization = Column(String(200), comment="Организация")
...
class Vehicle(Base):
    ...
    __table_args__ = (
        Index('idx_vehicle_original', 'original_name', unique=True),
    )
...
class ProviderTemplate(Base):
    ...
    Поддерживает три типа подключения:
    ...
    # Тип подключения: "file" (загрузка из файла), "firebird" (загрузка из БД Firebird) или "api" (загрузка через API)
    connection_type = Column(String(50), default="file", nullable=False, comment="Тип подключения: file, firebird, api")
    ...
    connection_settings = Column(Text, comment="JSON настройки подключения (для connection_type=firebird или api)")
...
class FuelCard(Base):
    ...
    card_number = Column(String(50), nullable=False, unique=True, index=True, comment="Номер топливной карты")
