...
/**
 * Устойчивый ключ строки. По индексу ключи брать нельзя: при удалении записи
 * React сопоставит строку с чужим состоянием поля ввода.
 */
let entryUid = 0
const nextEntryUid = () => ++entryUid
...
const entriesFromText = (text) => {
  ...
  try {
    ...
    if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
      return Object.entries(parsed).map(([key, value]) => ({ _uid: nextEntryUid(), key, value }))
    }
  } catch {
...
const FuelTypeMapping = ({ text, onTextChange }) => {
  ...
  const addEntry = () => changeEntries([...entries, { _uid: nextEntryUid(), key: '', value: '' }])
            ...
            {entries.length === 0 ? (
              ...
              <div className="fuel-mapping-rows">
                {entries.map((entry, index) => (
                  <div key={entry._uid} className="fuel-mapping-row">
                    <input
                      type="text"
                      value={entry.key}
